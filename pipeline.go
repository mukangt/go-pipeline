...
// GetStageIndexByName returns the index of a stage by its name
// Returns -1 if the stage is not found
func (p *Pipeline) GetStageIndexByName(name string) int {
...
// RunFromStageWithRequest executes the pipeline starting from a specified stage index with a custom request
// stageIndex: the zero-based index of the stage to start execution from
// request: the request object to use for pipeline execution
func (p *Pipeline) RunFromStageWithRequest(stageIndex int, request *Request) *Result {

	if len(p.Stages) == 0 {
		return &Result{Error: fmt.Errorf("pipeline contains no stages")}
	}

	if stageIndex < 0 || stageIndex >= len(p.Stages) {
		return &Result{Error: fmt.Errorf("invalid stage index %d, must be between 0 and %d", stageIndex, len(p.Stages)-1)}
	}

	// Initialize progress tracking if configured
	var progressTicker *time.Ticker
	if p.expectedDuration > 0 && p.tick > 0 {
		progressTicker = time.NewTicker(p.tick)
		progressCtx, cancelProgress := context.WithCancel(context.Background())
		p.cancelProgress = cancelProgress
		go p.updateProgress(progressTicker, progressCtx)
	}

	// Setup output buffering
	outputBuffer, exists := buffersMap.get(p.Name)
	if !exists {
		return &Result{Error: fmt.Errorf("failed to initialize output buffer for pipeline %s", p.Name)}
	}

	drainCtx, cancelDrain := context.WithCancel(context.Background())
	p.cancelDrain = cancelDrain
	go outputBuffer.drainBuffer(drainCtx)
	...
	p.status(fmt.Sprintf("begin from stage %d: %s", stageIndex, p.Stages[stageIndex].Name))
	...
	// Execute stages starting from the specified index
	for i := stageIndex; i < len(p.Stages); i++ {
		stage := p.Stages[i]
		stage.index = i
		...
		if result.Error != nil {
			...
			// p.status(fmt.Sprintf("stage %s at index %d encountered error", stage.Name, i))
			return result
		}
...
// Run the pipeline with request. The stages are executed in sequence while steps may be concurrent or sequential.
func (p *Pipeline) RunWithRequest(request *Request) *Result {
